...
public class DeleteCommandExecutor implements CommandExecutor, TabCompleter {
    private AdHomePlugin plugin;
    private DataStore store;
    private MessageUtil mUtil;

    DeleteCommandExecutor(AdHomePlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!sender.hasPermission("adhome.command.deletehome")) {
        ...
        if (home == null) {
            ...
            store.deleteHome(player, home.getName());
            mUtil.sendDeleteHome(player, home);
            plugin.cachedHomeNames.get(player).remove(home.getName());
        }
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (sender instanceof Player && sender.hasPermission("adhome.command.deletehome")) {
            Player player = (Player) sender;
